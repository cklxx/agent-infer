...
Last refreshed: 2026-05-09 EOD+163 (**🚫 P1.6 QKV projection packing KILLED `4d5f870`** — env-on TTFT p50 1979.5ms vs env-off 1981.4ms = -0.1% flat license fail + r3 server failure (43 ok / 88 failed) due to packed weight 缩 KV headroom under c=4 4k load(**新型 anti-pattern 候选 #27 memory-cost-shadow**); **🚫 P1.4 TileLang FP8 decode wire KILLED `51dd5b2`** substrate 语义不对齐(anti-pattern 候选 #26 same-output-but-garbage); **🚫 P1.3 quantized fused_mlp KILLED `edacfe7`** TTFT +7.3% regression(launch reduction saturated); **🎉 P1.2 W4A8 Marlin scratch hoist LANDED `ca0673b`** TTFT p50 1638.8ms / ITL p50 -13% vs P1.0;**🎉 P1.0 hybrid W4A8 prefill LANDED `9773904`** TTFT -31.5% env-gated;**P0.0 Phase 1.A nvtx decomp `aaf0b55`** prefill 97% dominant;**B3 Step 2 PrefixAwareAdmission LANDED `b85929b`** -24.2% TTFT;**战略 signal — ops-layer SOLID-grade saturated at current scope**(3 consecutive KILLs distinct failure modes:P1.3 saturated launch / P1.4 substrate semantic / P1.6 memory-cost-shadow per `8047072` strategic synthesis);next axis = **P0.0 Phase 1.B SGLang baseline re-verify**(brief drafted `/tmp/codex-brief-p0.0-phase1b-sglang-reverify.txt`,await user direction)or architectural pivot (multi-GPU / model-tier / distributed);215+ commits since 2026-05-08 EOD). Priority order canonical in [`ROADMAP.md` §Next-Model Priority Order](../ROADMAP.md#next-model-priority-order). Anchor doc for resumption: [`docs/research/2026-05-08-eod67-final-state-anchor.md`](research/2026-05-08-eod67-final-state-anchor.md). Pickup queue: [`docs/plans/codex-pickup-queue-2026-05-09.md`](plans/codex-pickup-queue-2026-05-09.md).
