...
Last refreshed: 2026-05-09 EOD+148 (**🚫 P1.4 TileLang FP8 decode wire KILLED `51dd5b2`** — substrate 语义不对齐:cubin build/link/launch OK + greedy_solo_vs_concurrent PASS,但真实 Qwen3-4B FP8 KV 输出退化(**新型 anti-pattern 候选 #26 same-output-but-garbage**:greedy_consistency invariant under "both-wrong-same-way"); **🚫 P1.3 quantized fused_mlp KILLED `edacfe7`** TTFT +7.3% regression env-on vs env-off (4-launch fallback already-optimized,launch reduction 假设 saturate); **🎉 P1.2 W4A8 Marlin scratch hoist LANDED `ca0673b`** TTFT p50 1638.8ms vs W4A16 -31.3% / ITL p50 11.41ms vs P1.0 hybrid -13.0%(P1.0 ITL tradeoff RESOLVED)/ out tok/s 224.95 vs W4A16 +17.5%;**🎉 P1.0 hybrid W4A8 prefill LANDED `9773904`** TTFT -31.5% env-gated `INFER_HYBRID_W4A8_PREFILL=1`;**P0.0 Phase 1.A nvtx decomp `aaf0b55`** prefill 97% dominant;**B3 Step 2 PrefixAwareAdmission LANDED `b85929b`** -24.2% TTFT;**战略 signal — ops-layer roadmap exhausted**(P1.3+P1.4 双连 KILL,P1.5 仅 cleanup);next axis = **P0.0 Phase 1.B SGLang baseline re-verify**(`/tmp/codex-brief-p0.0-phase1b-sglang-reverify.txt` brief drafted,await user direction);210+ commits since 2026-05-08 EOD). Priority order canonical in [`ROADMAP.md` §Next-Model Priority Order](../ROADMAP.md#next-model-priority-order). Anchor doc for resumption: [`docs/research/2026-05-08-eod67-final-state-anchor.md`](research/2026-05-08-eod67-final-state-anchor.md). Pickup queue: [`docs/plans/codex-pickup-queue-2026-05-09.md`](plans/codex-pickup-queue-2026-05-09.md).
