...
__global__ void dsv4_fp4_grouped_gemv_batch_kernel(
    const uint64_t* __restrict__ weight_ptrs,
    const uint64_t* __restrict__ scale_ptrs,
    const __nv_bfloat16* __restrict__ input,
    __nv_bfloat16* __restrict__ output,
...
__global__ void dsv4_fp8_grouped_gemv_pair_batch_kernel(
    ...
    int scale_cols)
{
    ...
    for (int k = tid_in_row; k < K; k += threads_per_row) {
        ...
        const float wa = dsv4_decode_fp8_e4m3(weight_a[row * K + k])
            * dsv4_decode_e8m0(scales_a[scale_row_offset + sc]);
        const float wb = dsv4_decode_fp8_e4m3(weight_b[row * K + k])
            * dsv4_decode_e8m0(scales_b[scale_row_offset + sc]);
        sum_a += wa * xv;
        sum_b += wb * xv;
    }
...
__global__ void dsv4_fp8_gemv_pair_batch_kernel(
    ...
    int scale_cols)
{
    ...
    if (tid_in_row == 0) {
        ...
        output_a[batch_idx * N + row] = __float2bfloat16(total_a);
        output_b[batch_idx * N + row] = __float2bfloat16(total_b);
    }
}

__global__ void dsv4_fp4_grouped_gemv_pair_batch_kernel(
    const uint64_t* __restrict__ weight_a_ptrs,
    const uint64_t* __restrict__ scale_a_ptrs,
    const uint64_t* __restrict__ weight_b_ptrs,
    const uint64_t* __restrict__ scale_b_ptrs,
...
__global__ void dsv4_fp8_route_gemv_pair_batch_kernel(
    const uint64_t* __restrict__ weight_a_ptrs,
    const uint64_t* __restrict__ scale_a_ptrs,
    const uint64_t* __restrict__ weight_b_ptrs,
    const uint64_t* __restrict__ scale_b_ptrs,
...
__global__ void dsv4_fp4_route_gemv_pair_batch_kernel(
    const uint64_t* __restrict__ weight_a_ptrs,
    const uint64_t* __restrict__ scale_a_ptrs,
    const uint64_t* __restrict__ weight_b_ptrs,
    const uint64_t* __restrict__ scale_b_ptrs,
...
__global__ void dsv4_fp4_gemv_pair_batch_kernel(
    const uint8_t* __restrict__ weight_a,
    const uint8_t* __restrict__ scales_a,
    const uint8_t* __restrict__ weight_b,
    const uint8_t* __restrict__ scales_b,
...
cudaError_t dsv4_fp8_gemv_pair_batch_cuda(
    ...
    cudaStream_t stream)
{
    ...
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, B);
    dim3 block(GEMV_THREADS);
    dsv4_fp8_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
        weight_a, scales_a, weight_b, scales_b, input, output_a, output_b,
        B, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
}

cudaError_t dsv4_fp4_gemv_pair_batch_cuda(
    const uint8_t* weight_a,
    const uint8_t* scales_a,
    const uint8_t* weight_b,
    const uint8_t* scales_b,
    ...
    cudaStream_t stream)
{
    ...
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, B);
    dim3 block(GEMV_THREADS);
    dsv4_fp4_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
        weight_a, scales_a, weight_b, scales_b, input, output_a, output_b,
        B, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
}
...
cudaError_t dsv4_fp8_grouped_gemv_batch_cuda(
...
cudaError_t dsv4_fp4_grouped_gemv_batch_cuda(
    const uint64_t* weight_ptrs,
    const uint64_t* scale_ptrs,
    const __nv_bfloat16* input,
    __nv_bfloat16* output,
...
cudaError_t dsv4_fp8_grouped_gemv_pair_batch_cuda(
    ...
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, max_count, num_experts);
    dsv4_fp8_grouped_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
...
cudaError_t dsv4_fp4_grouped_gemv_pair_batch_cuda(
    const uint64_t* weight_a_ptrs,
    const uint64_t* scale_a_ptrs,
    const uint64_t* weight_b_ptrs,
    const uint64_t* scale_b_ptrs,
    ...
    int scale_cols,
    cudaStream_t stream) {
    if (num_experts <= 0 || max_count <= 0 || N <= 0 || K <= 0 || (K & 1) != 0) {
        return cudaSuccess;
    }
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, max_count, num_experts);
    dsv4_fp4_grouped_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
        weight_a_ptrs, scale_a_ptrs, weight_b_ptrs, scale_b_ptrs, input,
        output_a, output_b, offsets, counts, expert_indices, max_count, N, K,
        scale_rows, scale_cols);
    return cudaGetLastError();
...
cudaError_t dsv4_fp8_route_gemv_pair_batch_cuda(
    const uint64_t* weight_a_ptrs,
    const uint64_t* scale_a_ptrs,
    const uint64_t* weight_b_ptrs,
    const uint64_t* scale_b_ptrs,
    ...
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, num_routes);
    dsv4_fp8_route_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
        weight_a_ptrs, scale_a_ptrs, weight_b_ptrs, scale_b_ptrs, input,
        output_a, output_b, route_meta, local_expert_start, experts_per_rank,
        num_routes, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
}

cudaError_t dsv4_fp4_route_gemv_pair_batch_cuda(
    const uint64_t* weight_a_ptrs,
    const uint64_t* scale_a_ptrs,
    const uint64_t* weight_b_ptrs,
    const uint64_t* scale_b_ptrs,
    ...
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, num_routes);
    dsv4_fp4_route_gemv_pair_batch_kernel<<<grid, block, 0, stream>>>(
        weight_a_ptrs, scale_a_ptrs, weight_b_ptrs, scale_b_ptrs, input,
        output_a, output_b, route_meta, local_expert_start, experts_per_rank,
        num_routes, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
