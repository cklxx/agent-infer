"""TileLang batch prefill HD64 paged attention.
...
HD64, BF16, causal, page_size=16. One kernel is AOT-specialized per
...
DSV4-mini-class size by extending the lockstep lists in this module,
cuda-kernels/build.rs, and cuda-kernels/src/ffi/attention.rs.

Tile / pipeline tunables (mirrors the HD128 prefill defaults; sm_89
re-tune is intentionally a separate later task):
...
# (num_q_heads, num_kv_heads) configurations the substrate emits.
# DSV4-mini = (16, 1). Extend here + the build.rs list + the matching FFI
# decl in lockstep when adding a new HD64 shape.
SUPPORTED_HEADS = (
    (16, 1),  # DSV4-mini-1B
)
...
def _make_kernel(num_q_heads: int, num_kv_heads: int):
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            # Causal-bound KV loop (see HD128 sister for derivation).
            ...
            # ~64x duplicate divmod + KV_indices gather the original
            ...
            for kn in T.Pipelined(T.ceildiv(kv_visible_end, BLOCK_N), num_stages=NUM_STAGES):
                ...
                # See sibling HD128 kernel + 2026-04-28 short-qlen NaN errors entry.
                ...
                # the acc_o rescale as a 2D T.Parallel — TileLang 0.1.9
                # LayoutInferencer constraint, see HD128 sibling comment.
